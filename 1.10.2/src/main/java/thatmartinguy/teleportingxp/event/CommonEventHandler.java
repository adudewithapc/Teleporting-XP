...
import thatmartinguy.teleportingxp.TeleportingXP;
...
{	
	...
	public void worldTicked(WorldTickEvent event)
	{
		...
		{
			if(event.world.getGameRules().getBoolean("teleportXP") || TeleportingXP.ignoreGamerule)
			{
				for(int i = 0; i < event.world.loadedEntityList.size(); i++)
				{
					if(event.world.loadedEntityList.get(i) instanceof EntityXPOrb)
					{
						...
						if(TeleportingXP.prioritizeNonCreative)
						{
